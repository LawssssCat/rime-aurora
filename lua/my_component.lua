  ...
  if(seg:has_tag("component")) then
    ...
    for component_name, component_info in pairs(component_run_info) do
      for i, function_name in pairs(component_func_names) do
        ...
        local duration_lst = component_info[function_name.."_duration_lst"]
        local info = {
          ...
          duration_lst = duration_lst and string.format("%0.4fs", duration_lst) or "-------",
        }
        -- show
        local text = component_name.."."..function_name
        local comment = string_helper.format("📊 count:\"{count}\", latest:\"{duration_lst}\", avg:\"{duration_avg}\", max:\"{duration_max}\", min:\"{duration_min}\"）", info)
        -- cand
